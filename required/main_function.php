<?php
	...
	function route1($path,$para=""){
		$str_para = '';
		if(!empty($para)){
			$str_para = "&".$para;
		}
		$result = MURL."index.php?route=".$path.$str_para;
		return $result;
	...
	function route($path,$para=""){
		...
		$url = MURL.'index.php?route='.$path;
	    $remove_http = str_replace('https://', '', $url);
	    $remove_http = str_replace('www.', '', $remove_http);
	    $split_url = explode('?', $remove_http);
	    // return $split_url[0];exit();
	    ...
	    for($i = 0; $i < count($split_parameters); $i++) {
	        ...
	        $add_str = (isset($final_split[2])?'/'.$final_split[2]:'');
	        $split_complete[$final_split[0]] = (isset($final_split[1])?$final_split[1]:'').$add_str;
	    }
	   
	    $id = (isset($split_complete['id'])?$split_complete['id']:'');
	    $text = (isset($split_complete['text'])?$split_complete['text']:'');
	    $cat = (isset($split_complete['cat'])?$split_complete['cat']:'');
	    $text = str_replace('"','',$text);
	    $text = str_replace("'",'',$text);
	    $text = str_replace("/",'_',$text);
	    $text = str_replace("%2F",'_',$text);

	    // $text = urlencode($text);
	    // $text = mb_strimwidth($text, 0, 50,'');
	    $text = '';
	    if($split_complete['route']=="cat"){
	    	$result = MURL.'category/'.$id.'/'.$text;
	    }else if($split_complete['route']=="detail"){
	    	$result = MURL.'news'.$cat.'/'.$id.'/'.$text;
	    }else if($split_complete['route']=="video/video_detail"){
	    	$result = MURL.'vdo'.'/'.$id.'/'.$text;
	    }else if($split_complete['route']=="video"){
	    	$result = MURL.'vdo';
	    }
	    // echo $split_complete['route'];exit();
	    // else if($split_complete['route']=="home"){
	    // 	$result = MURL.'หน้าหลัก';
	    // }
	    else{
	...
	function route2($path,$para=""){
	    ...
	    for($i = 0; $i < count($split_parameters); $i++) {
	        $final_split = explode('=', $split_parameters[$i]);
	        $add_str = (isset($final_split[2])?'/'.$final_split[2]:'');
	        $split_complete[$final_split[0]] = $final_split[1].$add_str;
	    }
	    ...
	    var_dump($split_complete['route']);
	    if($split_complete['route']=="cat"){
	    	$result = MURL.'หมวดหมู่/'.$id.'/'.$text;
	    }else if($split_complete['route']=="detail"){
	    	$result = MURL.'ข่าว'.$cat.'/'.$id.'/'.$text;
	    }else if($split_complete['route']=="video/video_detai"){
	    	$result = MURL.'วีดีโอ'.'/'.$id.'/'.$text;
	    }
	    // echo $split_complete['route'];exit();
	    // else if($split_complete['route']=="home"){
	    // 	$result = MURL.'หน้าหลัก';
